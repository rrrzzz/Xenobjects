...
namespace SplineMesh 
{
    ...
    public class SplineMeshTiling : MonoBehaviour 
    {
        ...
        public List<Vector2> _initialScales = new List<Vector2>();
        ...
        [Button]
        public void ScaleToFull(float scaleDuration = 2, float delayInterval = 0.1f)
        {
            ScaleSpline(true, scaleDuration, delayInterval);
            if (isInterpolating)
            {
                InterpolatePosDir();
            }
        ...
        [Button]
        public void ScaleToFullImmediate()
        {
        ...
        [Button]
        public void ScaleToZeroSmooth(float scaleDuration = 2, float delayInterval = 0.1f)
        {
            ScaleSpline(false, scaleDuration, delayInterval);
        }
        
        private void ScaleSpline(bool scaleToFull, float scaleDuration, float delayInterval)
        {
            foreach (var t in spline.nodes)
            {
                if (scaleToFull)
                {
