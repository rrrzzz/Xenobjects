...
public class RadialSpawner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Time.realtimeSinceStartup - _spawnTime > spawnInterval)
        {
            ...
            float degreesInRad = Mathf.Deg2Rad * _currentDegrees;

            Vector3 spawnPos = new Vector3(
                Mathf.Cos(degreesInRad) * radius + _centerPos.x,
                _centerPos.y + height,
                Mathf.Sin(degreesInRad) * radius + _centerPos.z
            );

            _spawnedObjects.Enqueue(
                Instantiate(_currentObjToSpawn, spawnPos, Quaternion.identity)
            );
